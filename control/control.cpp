...
vec radiance(const vector<sphere> &spheres, const ray &the_ray, int depth) noexcept
{
	...
	if (hit_sphere.reflection == reflection_type::DIFFUSE)
	{
		...
		const double r1 = std::future<double>(std::async([](double a, double b, double c) {return a * b * c; }, 2.0, PI, get_random_number())).get();
		const double r2 = std::future<double>(std::async([](double a) {return a; }, get_random_number())).get();
