// CaPScw1.cpp : Defines the entry point for the console application.
//
...
const double nc = 1;
const double nt = 1.5;
const double nntISinto = nc / nt;
const double nntISNOTinto = nt / nc;
...
vec radiance(const vector<sphere> &spheres, const ray &the_ray, int depth) noexcept
{
	...
	const double max_reflection = max({ colour.x, colour.y, colour.z });
	if (depth > MAX_DEPTH)
	{
	...
	if (hit_sphere.reflection == reflection_type::DIFFUSE)
	{
		...
		const vec new_direction = (u * cos(r1) * sqrt(r2) + v * sin(r1) * sqrt(r2) + w * sqrt(1 - r2)).normal();
	...
	double nnt = into ? nntISinto : nntISNOTinto;
...
int main(int argc, char **argv)
{
	...
	const ray camera(vec(50, 52, 295.6), vec(0, -0.042612, -1).normal());
	const vec cx = vec(0.5135);
	const vec cy = (cx.cross(camera.direction)).normal() * 0.5135;
	vec r;
	vec direction;
		...
		for (size_t x = 0; x < dimension; ++x)
		{
			for (size_t sy = 0, i = (dimension - y - 1) * dimension + x; sy < 2; ++sy)
			{
				for (size_t sx = 0; sx < 2; ++sx)
				{
					...
					{
						r1 = 2 * get_random_number();
						r2 = 2 * get_random_number();
						dx = r1 < 1 ? sqrt(r1) - 1 : 1 - sqrt(2 - r1);
						dy = r2 < 1 ? sqrt(r2) - 1 : 1 - sqrt(2 - r2);
						direction = cx * static_cast<double>(((sx + 0.5 + dx) / 2 + x) / dimension - 0.5) + cy * static_cast<double>(((sy + 0.5 + dy) / 2 + y) / dimension - 0.5) + camera.direction;
